...
int getFeatureMaps(const cv::Mat image, const int k, CvLSVMFeatureMapCaskade **map)
{
    ...
    dx    = cv::Mat(height, width, CV_32FC3);
    dy    = cv::Mat(height, width, CV_32FC3);
    ...
    cv::filter2D(image, dx, image.depth(), kernel_dx, cv::Point(-1, 0));
    cv::filter2D(image, dy, image.depth(), kernel_dy, cv::Point(0, -1));
